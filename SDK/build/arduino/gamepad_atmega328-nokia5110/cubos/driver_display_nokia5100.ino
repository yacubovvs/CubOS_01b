...
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
*/
#define DISPLAY_DRIVER_CONTRAST  60
...
void setup_displayDriver(){
    ...
    setContrast(DISPLAY_DRIVER_CONTRAST); // Good values range from 40-60
}
...
*/
 ...
 // We'll use SPI to transfer data. Faster!
//#include "driver_display_nokia5100.cpp" // We'll use SPI to transfer data. Faster!
...
byte displayMap[SCREEN_WIDTH * SCREEN_HEIGHT / 8] = {
...
void lcdFunTime()
{
  ...
  // white.
  //setPixel(random(0, SCREEN_WIDTH), random(0, SCREEN_HEIGHT));
  ...
  //setRect(0, 0, x, SCREEN_HEIGHT, 1, BLACK);
  ...
  clearDisplay(WHITE);
  setStr("Finish", 0, SCREEN_HEIGHT-8, BLACK);
...
void setPixel(int x, int y, boolean bw)
{
  ...
  if ((x >= 0) && (x < SCREEN_WIDTH) && (y >= 0) && (y < SCREEN_HEIGHT))
  {
    ...
    if (bw) // If black, set the bit.
      displayMap[x + (y/8)*SCREEN_WIDTH] |= 1<<shift;
