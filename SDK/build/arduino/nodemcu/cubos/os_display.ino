...
void drawRect_custom( int x0, int y0, int x1, int y1, int x2, int y2, int x3, int y3, boolean fill){
  if (fill){
    ...
    int min_x = min(min(x0, x1), min(x2, x3));
    int max_x = max(max(x0, x1), max(x2, x3));
    int min_y = min(min(y0, y1), min(y2, y3));
    int max_y = max(max(y0, y1), max(y2, y3));
